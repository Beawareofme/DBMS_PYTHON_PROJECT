...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/hmdb'
db=SQLAlchemy(app)
...
def signup():
    if request.method == "POST":
        ...
        new_user=db.engine.execute(f"INSERT INTO `user` (`username`,`usertype`,`email`,`password`,`number`) VALUES ('{username}','{usertype}','{email}','{encpassword}','{number}')")
